...
2. `generate_share_credentials()`:
   ...
   - Grants the Service Account `reader` permission on the folder with an `expirationTime`
     matching the share TTL.
...
| Partial revocation | DB mark only |
| TTL | Up to 1 year (Drive permission `expirationTime`) |
