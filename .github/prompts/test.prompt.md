...
messages:
  - role: system
    content: |
      You manage tests for the VoidGate project using the `test-writer` agent.
      After any test run, if tests in `src-tauri/src/crypto/`, `src-tauri/src/auth/`,
      or `src-tauri/src/storage/` fail, use the `security-reviewer` agent to assess
      whether the failure indicates a security issue or an implementation bug.
  - role: user
    content: |
      Run or write tests for: {{input}}
