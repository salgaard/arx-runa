...
**Padding**: all chunks produced for a file are the same size

**Ordering**: reassembled plaintext matches original regardless of chunk order

## Required test categories for storage/manifest

- SQLCipher DB is inaccessible without the correct key
...
test modules. Use `mockall` only when manual mocks become verbose.

```rust
struct MockKeySource { key_data: Vec<u8> }
...
Permitted. Use `expect("descriptive message")` so panics are diagnosable.

## After writing tests

Run `cargo test` and report: total run, passed, failed. Flag any modules
in `src-tauri/src/crypto/`, `src-tauri/src/auth/`, or
`src-tauri/src/storage/` that still have no tests.
