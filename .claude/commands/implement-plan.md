...
- **Scope-driven reviewers.** Which reviewers run is determined here by actual changed files — not by plan frontmatter flags.
...
**Implementer-agnostic**: this command runs on any CLI agent that can read `.claude/` resources. It assumes no specific model and no human in the loop beyond the hard gates listed explicitly in Step 3.
...
8. **Implementation-mode gate**:
   ...
   - If `delegated`: verify Section 9 exists with explicit delegation boundaries.
...
- `finding-classifier` and `problem-solver` govern remediation structure; `rust-implementer` executes approved remediation payloads including any design-doc updates.
...
7. **Security review**: if any files under `src-tauri/src/{crypto,auth,storage}/` were changed, invoke `security-reviewer`. Pass relevant `DIGEST_SLICE` and the security concerns noted in plan Section 6b. If no security-path files changed, skip and record in Implementation Log.
   - **Drift check (always runs)**: compare files actually modified under `src-tauri/src/{crypto,auth,storage}/` against the plan's **Section 6b (Security-sensitive paths)**. If any sensitive file was touched that the plan did not anticipate, invoke Plan-deviation protocol and halt.
...
12. **Problem-solver invocation**: if `ACTIONABLE_NOW` is empty, continue to Testing. Otherwise:
    - Pass the full `design_challenge_ledger` from `CLASSIFIED_FINDINGS` to `problem-solver` as `design_challenge_entries`. Problem-solver evaluates each challenge and decides whether to accept or reject it — its `SOLUTION_PACK` includes the rationale and, for accepted challenges, a design-doc update step.
    ...
    - In `delegated` mode: invoke `rust-implementer` with `SOLUTION_PACK`. Solutions may include design-doc update steps — `rust-implementer` implements these alongside code changes. Require `IMPLEMENTATION_RESULT`. If any item returns `BLOCKED`, invoke Plan-deviation protocol and halt.
    - In `direct` mode: implement fixes directly or delegate selectively using the same `SOLUTION_PACK` contract. Design-doc updates are part of the same implementation pass.
...
- Do not skip the design-doc sync gate in Step 6 when accepted challenges exist — design documents must reflect implemented challenges before status is set to `implemented`.
