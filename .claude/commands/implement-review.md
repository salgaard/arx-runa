...
4. `security-reviewer` (when needed)
...
2. Re-run `security-reviewer` on changed shards under `auth/`, `crypto/`, `storage/` or when risk indicators appear.
3. Use compact structured findings only (same schema as Phase 1).
4. If actionable CRITICAL/HIGH remain in a shard:
...
6. Max iterations: 8 remediation rounds.
7. Reviewer-only loops are not allowed when actionable CRITICAL/HIGH remain.
