...
- **macOS**: `DiskArbitration` framework — `DARegisterDiskAppearedCallback` / `DARegisterDiskDisappearedCallback` via raw FFI through the `core-foundation` crate; callbacks dispatched on a dedicated `CFRunLoop` thread
...
- `MacOsDeviceMonitor` — `DiskArbitration` framework via `core-foundation` FFI
